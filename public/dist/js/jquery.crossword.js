...
(function($){
	$.fn.crossword = function(entryData) {
			...
			*/
			...
			this.after('<div class="six wide column" id="puzzle-clues"><h2>Horizontal</h2><ul id="across"></ul><h2>Vertical</h2><ul id="down"></ul></div>');
			...
			var puzInit = {
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {		
						...
						$('#' + puzz.data[i].orientation).append('<li tabindex="1" data-position="' + i + '">' + "<b>" + puzz.data[i].position + ". </b>" +  puzz.data[i].clue + '</li>'); 
					}				
				...
				*/
				checkAnswer: function(e) {
					...
					if(valToCheck === currVal && !solved.includes(valToCheck)){	
						$('.active')
							.addClass('done')
							//.prop('disabled', true)
							.removeClass('active');
						...
						solvedToggle = true;
						console.log(solved.length + " / " + puzz.data.length);
						if (solved.length === puzz.data.length) {
							alert('YEI GANASTE')
						}
						return;
					}
